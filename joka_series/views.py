...
def search(request):
    ...
    addUrl      = f"search-series?searchword={quote_plus(searchTerm)}&searchphrase=all&limit=0"
    finalUrl    = baseUrl + addUrl
    res         = requests.get(finalUrl)
    soup        = BeautifulSoup(res.content,'html.parser')
...
def detail(request,series):
    ...
    seriesDesc  = [para.get_text() for para in allPara
                     if not (para.get_text().startswith('Download')
